...
describe('ErrorCatchingInterceptor', () => {
  ...
  it('should pass through if the response is HttpResponse', () => {
    ...
    http.get('https://jsonplaceholder.typicode.com/posts?userId=').subscribe((res: HttpResponse<any>) => {
      expect(res).toBeTruthy();
      expect(res.status).toBe(mockData.status);
      expect(res.body).toEqual(mockData.body);
    });

    const request = httpMock.expectOne('https://jsonplaceholder.typicode.com/posts?userId=');
